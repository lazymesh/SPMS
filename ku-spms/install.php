...
?>";
 ...
 /*   $table1 = mysql_query("CREATE TABLE `company` (
  ...
  `grade` varchar(10) NOT NULL default 'not graded',
  `svid` int(10) NOT NULL default '0',
  `gradedon` varchar(50) NOT NULL default '',
  PRIMARY KEY  (`ID`),
  KEY `status` (`status`)
) ENGINE=MyISAM");
...
CREATE TABLE `roles_assigned` (
  ...
  PRIMARY KEY  (`ID`)
    ...
    $ins = mysql_query("INSERT INTO settings (name,subtitle,locale,timezone,dateformat,template,mailnotify,mailfrom,mailfromname,mailmethod,mailhost,mailuser,mailpass) VALUES ('Kathmandu University','Student Project Management System','$locale','$timezone','d.m.Y','standard',1,'ku-spms@gmail.com','Kathmandu University','smtp','smtp.gmail.com','ku-spms@gmail.com','kuspms123')");
    ...
    $usr = new user();
    $usrid = $usr->add($user, "", "ku-spms@gmail.com", $pass);
    if (!$usrid) {
    ...
    $userrid = $rolesobj->add("Supervisor", array("add" => 1, "edit" => 1, "del" => 1 , "close" => 1), array("add" => 1, "edit" => 1, "del" => 1, "close" => 1), array("add" => 1, "edit" => 1, "del" => 1, "close" => 1), array("add" => 1, "edit" => 1, "del" => 1, "close" => 1), array("add" => 1, "edit" => 1, "del" => 1), array("add" => 1, "edit" => 1, "del" => 1, "read" => 1), array("add" => 1), array("add" => 0));

    $clientrid = $rolesobj->add("Student", array("add" => 1, "edit" => 1, "del" => 0, "close" => 0), array("add" => 1, "edit" => 1, "del" => 1, "close" => 0), array("add" => 1, "edit" => 1, "del" => 1, "close" => 0), array("add" => 1, "edit" => 1, "del" => 1, "close" => 1), array("add" => 1, "edit" => 1, "del" => 0), array("add" => 1, "edit" => 1, "del" => 1, "read" => 1), array("add" => 1), array("add" => 0));
