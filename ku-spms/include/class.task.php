...
class task
{
	...
	try{
		...
		$text= "the task \"".$nameproject[0]."\" has been created.";
		$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$nameproject[1])");
	...
	try{
		$subject="task edited";
		$text= "the task \"".$nameproject[0]."\" has been edited.";
		$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$nameproject[1])");
		while($eml = mysql_fetch_array($email)){
			$e = $eml[0];
			$name = $eml[1];
	...
	try{
		...
		$text= "the task \"".$nameproject[0]."\" has been deleted.";
		$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$nameproject[1])");
		while($eml = mysql_fetch_array($email)){
			$e = $eml[0];
			$name = $eml[1];
	...
	try{
		$subject="task opened";
		$text= "the task \"".$nameproject[0]."\" has been opened.";
		$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$nameproject[1])");
		while($eml = mysql_fetch_array($email)){
			$e = $eml[0];
			$name = $eml[1];
	...
	try{
		$subject="task closed";
		$text= "the task \"".$nameproject[0]."\" has been closed.";
		$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$nameproject[1])");
		while($eml = mysql_fetch_array($email)){
			$e = $eml[0];
			$name = $eml[1];
    ...
    private function getNameProject($id)
    {
        ...
        $list = $nam[1];
        $sel2 = mysql_query("SELECT project, name FROM tasklist WHERE ID = $list");
