...
class datei
{
    ...
    function addFolder($parent, $project, $folder, $desc, $visible = "")
    {
		...
		try{		
			...
			$text= $folderOrig." has been added as a new folder, please visit KU-SPMS account";
        ...
        if ($del)
	{
		...
		try{
			...
			$text= $folder["name"]." has been deleted, please visit KU-SPMS account";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
    ...
    function edit($id, $title, $desc, $tags)
    {
		...
		try{
			$subject="folder edited";
			$text= $title." has been edited, please visit KU-SPMS account";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
    ...
    private function add_file($name, $desc, $project, $milestone, $tags, $datei, $type, $title, $folder = 0, $visstr = "")
    {
		...
		try{
			...
			$text= $name." file has been created, please visit KU-SPMS account";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
