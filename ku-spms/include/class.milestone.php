...
class milestone
{
    ...
    function add($project, $name, $desc, $end, $status)
    {
		...
		try{
			...
			$text= $name." milestone has been created, please visit KU-SPMS account";
    ...
    function edit($id, $name, $desc, $end)
    {
		...
		try{
			$subject="milestone edited";
			$text= $name." milestone has been edited, please visit KU-SPMS account";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
    ...
    function del($id)
    {
		...
		try{
			...
			$text= $name." milestone has been deleted, please visit KU-SPMS account";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
    ...
    function open($id)
    {
		...
		try{
			$subject="milestone opened";
			$text= $name." milestone has been opened, please visit KU-SPMS account";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
    ...
    function close($id)
    {
		...
		try{
			$subject="milestone closed";
			$text= $name." milestone has been closed, please visit KU-SPMS account";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
