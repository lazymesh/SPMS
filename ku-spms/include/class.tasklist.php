...
class tasklist
{
    ...
    */
    ...
    function add_liste($project, $name, $desc, $access = 0, $milestone = 0)
    {
		...
		try{
			$settings=mysql_query("SELECT * FROM projekte WHERE ID=$project");
		     	$settings=mysql_fetch_array($settings);
			$subject="project task-list opened";
			$text= "task-list \"".$name."\" for the project \"".$settings['name']."\" has been created.";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
				$this->notice->send_mail($e,$subject,$text,$name);
			}
		}
...
//echo $sel;die();
	 	...
	 	try{
			$settings=mysql_query("SELECT * FROM projekte WHERE ID=$proj");
		     	$settings=mysql_fetch_array($settings);
			$subject="project task-list edited";
			$text= "task-list \"".$name."\" for the project \"".$settings['name']."\" has been edited.";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$proj)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
				$this->notice->send_mail($e,$subject,$text,$name);
	...
	$del = mysql_query("DELETE FROM tasklist WHERE ID = $id LIMIT 1");
		...
		try{
			$settings=mysql_query("SELECT * FROM projekte WHERE ID=$proj");
		     	$settings=mysql_fetch_array($settings);
			$subject="project task-list deleted";
			$text= "task-list \"".$name."\" for the project \"".$settings['name']."\" has been deleted.";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$proj)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
    ...
    function open_liste($id)
    {
		...
		try{
			$settings=mysql_query("SELECT * FROM projekte WHERE ID=$project");
		     	$settings=mysql_fetch_array($settings);
			$subject="project task-list opened";
			$text= "task list \"".$name."\" for the project \"".$settings['name']."\" has been opened.";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
    ...
    function close_liste($id)
    {
		...
		try{
			$settings=mysql_query("SELECT * FROM projekte WHERE ID=$project");
		     	$settings=mysql_fetch_array($settings);
			$subject="project task-list closed";
			$text= "task list \"".$name."\" for the project \"".$settings['name']."\" has been closed.";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$project)");
			while($eml = mysql_fetch_array($email)){
				$e = $eml[0];
				$name = $eml[1];
