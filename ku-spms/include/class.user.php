...
class user
{
     ...
     * @param string $company faculty of the member
     * @param string $pass Password
     * @param string $locale Localisation
	 * @param float $rate Hourly rate
     * @return int $insid ID of the newly created member
     */
    function add($name, $role, $email, $pass, $company, $locale = "", $address1, $address2, $country, $tel1)
    {
        ...
        $ins1 = mysql_query("INSERT INTO user (`name`,`role`,`email`,`faculty`,`pass`,`gender`,`adress`,`tel1`) VALUES ('$name','$role','$email','$company','$pass','$gender','$address1',$tel1)");
	...
	try{
		$subject="user added";
		$text= "you have been added in KU-SPMS";
		$this->notice->send_mail($email,$subject,$text, $name);
		}
    ...
    function edit($id, $name, $realname, $email, $tel1, $tel2, $company, $zip, $gender, $url, $address1, $address2, $state, $country, $tags, $locale, $avatar = "", $rate = 0.0)
    {
        ...
        if ($avatar != "")
        {
            $upd = mysql_query("UPDATE user SET name='$name',email='$email',tel1='$tel1', tel2='$tel2',faculty='$company',zip='$zip',gender='$gender',url='$url',adress='$address1',adress2='$address2',state='$state',country='$country',tags='$tags',locale='$locale',avatar='$avatar',rate='$rate' WHERE ID = $id");
        }
        else
        {
            $upd = mysql_query("UPDATE user SET name='$name',email='$email', tel1='$tel1', tel2='$tel2', faculty='$company',zip='$zip',gender='$gender',url='$url',adress='$address1',adress2='$address2',state='$state',country='$country',tags='$tags',locale='$locale',rate='$rate' WHERE ID = $id");
        }
        if ($upd)
        {
	...
	try{
		...
		$text= "your profile has been updated in KU-SPMS";
		$this->notice->send_mail($email,$subject,$text, $name);
	}
	catch(userException $e){}
            return true;
    ...
    function editpass($id, $oldpass, $newpass, $repeatpass)
    {
        ...
        $chk = mysql_query("SELECT ID, name, email FROM user WHERE ID = $id AND pass = '$oldpass'");
    ...
    function del($id)
    {
        ...
        $chk = mysql_query("SELECT name, email FROM user WHERE ID = $id");
        $chk = mysql_fetch_row($chk);
        $name = $chk[0];
	$email = $chk[1];
	...
	try{
		...
		$text= "your profile has been deleted in KU-SPMS";
		$this->notice->send_mail($email,$subject,$text, $name);
	}
	catch(userException $e){}
            return true;
    ...
    function getProfile($id)
    {
        ...
        if (!empty($profile))
        {
            ...
            if (isset($profile["faculty"]))
            {
                $profile["faculty"] = stripslashes($profile["faculty"]);
            }
    ...
    function getAllUsers($lim = 10)
    {
        ...
        while ($user = mysql_fetch_array($sel2))
        {
            ...
            $user["faculty"] = stripslashes($user["faculty"]);
