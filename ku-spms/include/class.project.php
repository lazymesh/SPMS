...
class project {
    ...
    function del($id)
    {
		...
		$settings=mysql_query("SELECT * FROM projekte WHERE ID=$id");
	     	$settings=mysql_fetch_array($settings);
		$subject="project deleted";
		$text= "The project \"".$settings['name']."\" has been deleted.";
    ...
    function open($id)
    {
		...
		$text= "The project \"".$nam."\" has been opened.";
		$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$id)");
		while($eml = mysql_fetch_array($email)){
			$e = $eml[0];
			$name = $eml[1];
    ...
    function close($id)
    {
		...
		$text= "The project \"".$nam."\" has been closed.";
		$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$id)");
		while($eml = mysql_fetch_array($email)){
			$e = $eml[0];
			$name = $eml[1];
    ...
    function assign($user, $id)
    {
		...
		try{
			$project = mysql_query("SELECT name FROM projekte WHERE ID=$id");
			$proj = mysql_fetch_array($project);
			$proj = $proj[0];
			$subject = "project added";
			$text = "The project \"".$proj."\" has been added. Please check in KUSPMS for detail.";
			$this->notice->send_mail($user["email"],$subject,$text,$user["name"]);
		}
		catch(projectException $e){}
            return true;
        } else {
