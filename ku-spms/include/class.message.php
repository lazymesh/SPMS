...
class message
{
    ...
    function add($project, $title, $text, $tags, $user, $username, $replyto, $milestone)
    {
		...
		try{
			$subject="message created/replied";
			$text= "there is a message titled \"".$title."\" for you.";
    ...
    function edit($id, $title, $text, $tags)
    {
		...
		try{
			$subject="message edited";
			$text= "the message \"".$title."\" has been edited.";
    ...
    function del($id)
    {
		...
		try{
			...
			$text= "the message \"".$msg[0]."\" has been deleted.";
			$email = mysql_query("SELECT U.email, U.name FROM user U WHERE U.ID IN (SELECT PA.user FROM projekte_assigned PA WHERE PA.projekt=$msg[1])");
			while($eml = mysql_fetch_array($email)){
				$e= $eml[0];
				$name = $eml[1];
				$this->notice->send_mail($e,$subject,$text,$name);
